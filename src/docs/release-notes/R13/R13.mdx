...
We now have examples showing how the different Brightlayer UI [Themes](/style/themes) affect various UI elements in an application.
...
Brightlayer UI now offers resources for working with ChartJS - an open source alternative to HighCharts. You can read more on our [Visualizations](/patterns/visualizations) page or on the [ChartJS](https://www.chartjs.org) documentation site.

### Component Library

We have been working to innersource new Brightlayer UI components for React Native applications - these are currently available as [@brightlayer-ui/react-native-components](https://www.npmjs.com/package/@brightlayer-ui/react-native-components). Documentation for these components can be found on [GitHub](https://github.com/brightlayer-ui/react-native-component-library/tree/dev/docs). This package is still in alpha, but a production version will be made available in an upcoming Brightlayer UI release.
