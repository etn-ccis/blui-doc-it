...
Brightlayer UI provides multiple icon color options to help communicate meaning, status, and priority within an interface. Available color variants include Primary (Blue), Error (Red), Warning (Yellow), Success (Green), Orange, Purple, and Neutral (Grayscale).

Each icon color has been carefully optimized for both light and dark themes to maintain visual consistency and accessibility across different viewing conditions. Color selections are designed to support semantic communication while helping users quickly identify important information, system states, or actions.

For non-text content such as icons, WCAG requires a minimum contrast ratio of 3:1 against adjacent colors to ensure sufficient visibility and recognition. Color values used throughout Brightlayer UI have been selected with this requirement in mind wherever possible. When a color alone cannot achieve the desired contrast, particularly with colors such as yellow that naturally have lower luminance contrast, additional visual techniques may be used to improve icon legibility and recognition.

### Recommended Usage

-   **Primary (Blue):** Default actions, navigation, and key interactive elements.
-   **Error (Red):** Error states, critical alerts, failures, or destructive actions.
-   **Warning (Yellow):** Warning states, cautionary information, and situations requiring attention.
-   **Success (Green):** Successful operations, confirmations, and positive system states.
-   **Orange:** High-priority notifications, maintenance states, or informational emphasis where warning semantics are not appropriate.
-   **Purple:** Specialized features, categories, or branded experiences that require visual distinction without implying status.
-   **Neutral (Grayscale):** Default, inactive, disabled, or low-emphasis icons.
