...
PX Blue provides a type scale that includes a range of styles to support the needs of your application. All styles and fonts are available in the design library. As PX Blue continues to grow, additional font patterns will be added.<ImageGrid images={[TypeScaleImg]} gridImageProps={{ xs: 12, sm: 8, md: 8 }} style={{marginTop: 32}}/>
<Divider fullScreen />

# Customization

Different products have different needs, and we do not expect these pre-defined styles will cover every potential use case. Slight customization of these styles, such as changing a font weight in a particular component, are acceptable and expected. In most cases, you should be able to adjust existing styles without having to define an entirely new style. If you find that you must create new styles (see below), they should still use the PX Blue font (Open Sans) and adhere to our color guidelines.<ImageGrid images={[LargerApplicationImg]} gridImageProps={{ xs: 12, sm: 8, md: 8 }} style={{marginTop: 32}} />
<Divider />
