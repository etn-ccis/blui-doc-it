...
Brightlayer UI provides a type scale that includes a range of styles to support the needs of your application. All styles and fonts are available in the design library. As Brightlayer UI continues to grow, additional font patterns will be added.
...
<ImageGrid
    ...
    caption={[
        `1: Do not rely solely on font weight changes to indicate an active state. Avoid having auxiliary information (e.g., the circled numbers on the right) compete for users' attention.`,
        `2: Use clues like background color and radio buttons to clearly communicate the state.`,
    ]}
/>
...
<ImageGrid
    ...
    caption={[
        '1: Be careful when using status colors for a large body of text, as they have reduced legibility compared to standard text colors.',
        '2: Shorter text, such as a list item tag, may be colored to reflect a certain status.',
    ]}
/>
