...
<ImageGrid
    ...
    regularWidth
    caption={[
        '1: Figure 1 shows how color should be selected on mobile devices. It is important to display the information this way since users do not have the roll over feature.',
        '2: Figure 2 shows the incorrect way to choose color on a mobile device. There are no indications of what the colors are and the selectable area is too small which will lead to increase in missclicks.',
    ]}
/>
...
<ImageGrid
    ...
    regularWidth
    caption={[
        '1: Figure 1 shows how  to properly display an action for a card. By having a visible "Read More" button it communicates to the user that an article is available to view.',
        '2: Figure 2 shows the incorrect way to dipsplay an action for a card. While the card is clickable it is not immediately clear that there is additional content available.',
    ]}
/>
