...
The foundations include the most basic visual elements on the page, such as [color](/style/color), [icons](/style/icon-guideline), [typography](/style/typography), etc. These typically have no behavior or interaction on their own, but instead are used to communicate information to a user, establish a brand, or lay the foundations for the Brightlayer UI components, pages, and workflows.
