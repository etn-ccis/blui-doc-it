...
Lists and tables are frequently used throughout PX Blue applications. Typically, lists are used to represent groups of similar data. They can display from simple name-value pairs in a sidebar all the way to complex timelines or device lists.
...
The scenarios in which lists may occur include:
...
-   There are no comparable data presents, such as data sets with different structures;
...
<ImageGrid
    ...
    regularWidth
    caption={
        'Whenever you have a list that may include both avatars and icons, make sure all the icons, with or without avatars, are center-aligned. If you are using interactive elements like checkboxes and radio buttons, they should also be center-aligned to accomodate ripples. Otherwise, all icons should be left-aligned.'
    }
/>

In the rare case where you need to display status but cannot fit in neither an avatar nor a status stripe, such as the mini-list inside a [scorecard component](https://pxblue-components.github.io/react/?path=/info/components-score-card--get-read-me-story), you may color-code your list item icons and labels. However, for accessibility concerns, you should never rely _solely_ on the color to convey the message.
...
Using tags is another way to display list status. They typically appear at the very end of a list item.

<ImageGrid
    images={[ContactsImage]}
    regularWidth
    caption={'This user list uses three status indicators: a status stripe, dot badges, and tags.'}
/>
