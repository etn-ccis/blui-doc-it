...
Lists emphasize on each individual items, while tables highlights the comparison between each individual data points. Tables work well on larger screens, but at a mobile size, they should collapse into a list for better formatting.

Tables are more "data visualisable". Do not structure the list as a table if your users don't examine the data "vertically".
...
Global list actions are the actions that manipulate the entire list, rather than treating each list item individually. The buttons sit in the app bar, a button row (desktop-only), or the panel header.
...
Common global list action includes:

-   Add a new list item
-   Sort and filter the list
-   Toggle a different view, such as grid view
-   Export or download the list data
...
Drag and drop is a special case for sorting the list. It re-orders the items in a list manually. This should be accomplished by placing a drag handle as the leftmost element in the row. The ability to re-order the list should be locked behind an Edit Mode toggle mechanism.
...
If there are actions that you would like to perform on individual list items, these can be included via icons or a 3-dot action menu (multiple actions) on each list item. Depending on the space available, you typically will want to use the action menu if you have more than three actions that can be taken.
...
Lists can be used to navigate a tree-like structure in an application. See also [the navigation pattern](/patterns/navigation).
