...
In general, there are three different types of input field verification: on blur, on change, and on page change.

### Verify Input on Blur

In order to not "push" the user too much, most text fields should remain "verify on blur". That is, most of the time, you should only verify user input when they stop typing.
...
On rare occasions when you have to communicate with a remote server to verify the user input (rather than just doing client-side checks), and there is a relatively small (less than 3) number of text fields, you may do the verification on page change. Some typical use cases of this would be the login screen where the user credentials are checked and discover new devices on a network.
