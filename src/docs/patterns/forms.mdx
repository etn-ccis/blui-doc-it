...
Conversely, if most of your fields are optional, you should only call out the required fields. These should be identified by an asterisk (\*) after the field label. Optionally, you can add a descriptor nearby that indicates that a \* means the field is required.
...
When the user attempts to submit by clicking on the submit button on their screen (desktop) or on their native keyboard (mobile), all the invalid fields should be highlighted with red error text.
...
In most cases, the submit button **should not** be disabled. The users should not be forced to search for what they are missing.
