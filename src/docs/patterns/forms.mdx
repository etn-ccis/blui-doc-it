...
In order to avoid distracting or confusing a user, most text fields should verify input when the field loses focus; when the user loses focus with invalid data, the error will appear.
...
Usually, a form will have a submit/next/finished button at the end. This button is typically placed at the bottom right corner of the form. If you are designing for a right-to-left language, this button should be at the bottom left.
...
<ImageGrid
    ...
    gridImageProps={{ sm: 12 }}
    caption={[
        'On mobile, if there are more than one input fields on the screen, the keyboard button should say "next".',
        '1: The user entered a valid input. Clicking "next" will autofocus the user to the next text field.',
        '2: The user entered a invalid input. Clicking "next" will display the error message but will not autofocus to the next text field.'
    ]}
/>

### Disable the Submit Button

When there are not a lot of text fields, the submit button can be disabled to indicate that there are some errors in the user input.

<ImageGrid
    images={[SubmitButtonOnChangeImage]}
    gridImageProps={{ sm: 12 }}
    caption={
        'If you have only one input field, and this field is verified on change, disable the submit button until the user entered the desired input.'
    }
/>
<ImageGrid
    ...
    gridImageProps={{ sm: 12 }}
    caption={
        'For the verification code screen, you can auto-submit the code once the user input reaches the desired length (2). After the verification code is recognized (3), the screen pauses for a short moment before auto-transit to the next page. The verify button is preserved for consistency.'
    }
/>
...
When you have a long form (more than three text fields) with a lot of text fields and the user forgets a required field, do not auto disable the submit button -- it will take them some effort to check what they are missing. Instead, allow the user to click the submit button, and highlight the errors for them.
