...
> This page is still a work in progress for projects using React Native and is subject to change in the future. [Let us know](/community/contactus) what you think.
...
<ImageGrid
    ...
    caption={[`CAUTION: Be careful not to place too many columns to overflow the table. If a horizontal scroll is necessary, place a horizontal scroll bar between rows and the footer / pagination area.`]}
    regularWidth
/>
...
<ImageGrid
    ...
    caption={[
        `If you do an eye tracking between when the user starts looking at their data set and when they decide to perform some actions, you would notice users' eye sights roam differently in these two structures.`,
...
For an industrial mobile application, users would often be on foot and out in the field when they interact with the app. They are paying more attention to their surroundings, so keep the mobile app interaction minimal.
...
If tables become necessary in your mobile application, pagination is especially helpful to reduce the load of both the server and the client-side render.
