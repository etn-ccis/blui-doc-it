...
Often, table data look highly similar between rows and can disorient your users. You may consider using animations in your applications to help users understand certain transitions.
...
<ImageGrid
    ...
    captionsUnderImages={[
        'Avoid using tables in dashboards. Use lists instead.',
        'Tables are usually the only main content on a page.',
    ]}
/>

A "tabular list" that attempts to combine tables and lists together might result in a design that does not pick up the advantages of either pattern.

<ImageGrid
    images={[TabularList]}
    captionsUnderImages={['Avoid combining tables and lists together, as this diminishes the value of both.']}
...
In general, we do not encourage tables on mobile platforms, as table-related interactions tend to be complicated for small view ports, and mobile phones also often lack the computational power to process massive data set.
...
Before you design a table structure for a mobile application, you should conduct user researches and have a clear understanding of users' intention. When they land on this page, Do they desire to do complicated cross-comparison between multiple rows and columns? Is a full table necessary?

## Breaking Down Tables into Lists

If a user only need a few parameters to identify the correct data entry and make decisions, a list with a few selected data points would suffice.

<ImageGrid
    images={[HomePagePreview, TableStackedAsList]}
    captionsUnderImages={[
        `On a crowded page, show a small portion preview of the table content, with the option to view the full table in a dedicated page.`,
        `Stacking each property into a list item, with the ability to click into a list item to view the full item details.`,
    ]}
/>
...
<ImageGrid
    ...
    captionsUnderImages={[
        `Pagination is often used to reduce the page load. If not conflicting with other gestures, consider using left / right swipes to go to the previous / next page.`,
    ]}
    gridContainerProps={{ style: { justifyContent: 'flex-start' } }}
/>
...
You may also consider embedding action gestures common for touch screens, such as long-press and swipe.
