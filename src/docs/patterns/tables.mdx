...
Occasionally, table rows may expand to allow users _take a quick peek_ into a small amount of row details.

<ImageGrid
    images={[ExpandedRow]}
    caption={[`Users can quickly glance into a small amount of row details.`]}
    regularWidth
/>
...
Tables facilitate users who want to work across multiple different properties of a data point. Users often navigate down a column, find the desired value, jump to another column, navigating further down, before they exit their path. This is called a [lawn mower pattern](https://www.nngroup.com/articles/lawn-mower-pattern/). On the other hand, lists often have a primary property — can be icons, avatars, or primary list texts — that the user would rely on for navigation, a scanning behavior known as the [layer cake pattern](https://www.nngroup.com/articles/layer-cake-pattern-scanning/).
