...
<ImageGrid
    ...
    regularWidth
    caption={[
        `An example login screen on mobile (1 & 3) and on desktop (2 & 4). On desktop, authentication workflows are typically presented in a Card.`,
    ]}
/>

Your authentication and registration workflow should work equally well on different screen sizes. On mobile, these should be presented as full-screen layouts, whereas on desktop, they should be presented in a card centered on the screen as an [overlay](/patterns/overlay).
...
If you do not need to restrict who can register for your application, you can allow users to self-register. The self-registration workflow includes the following screens:
...
If you need to restrict who is able to use your application, you may choose to go with a invitation-based registration. In this situation, a system admin sends an email to authorized users inviting them to register. The invitation-based registration workflow includes the following screens:
...
Joining by passcode occurs when anyone with an organization identifier and passcode can join an organization. This workflow includes the following screens:
...
<ImageGrid
    ...
    caption={['1: Enter email address', '2: Reset instructions sent', '3: Create a new assword', '4: Success']}
/>

**Change password**

When the users are logged in, they have the option to update or change their password by going through the change password workflow, typically triggered within the user settings. This workflow includes the following screens:

-   Change password
-   Success

<React.Fragment>
    <ImageGrid images={[ChangePassword, ChangePasswordWelcome]} caption={['1: Change password', '2: Success']} />
    <Divider />
</React.Fragment>
...
We recommend using the following packages to construct these workflows:
