...
Most applications will require some form of user authentication to ensure that only authorized users have access as well as a mechanism for users to be added to the application.
...
The workflows should follow the rules specified by our [forms and validation](/patterns/forms) pattern, and multi-step workflows should be connected using our [steppers](/patterns/steppers) pattern.
...
The user authentication contains four primary workflows:
...
/>
<ImageGrid images={[RegistrationDesktop]} caption={'A registration screen example on desktop.'} regularWidth/>
...
Joining by passcode occurs when anyone with an organization identifier and passcode can join an organization. This workflow includes the following steps:
...
<ImageGrid
    ...
    caption={[
        ...
        '3: Success: request sent',
        '4: Success: organization joined',
    ]}
/>
...
The login screen should always offer some workaround if a user has some trouble getting authorized, either because they don't have an account, or that they forget their credentials. In your login screen, there should be buttons that can initiate those extra functions, such as Registration and Forgot Password (see below).
...
<ImageGrid
    ...
    caption={[
        ...
        '2: Optionally, you may add in a simple geometrical background image with a white gradient background.',
...
The login screen for an application should also present users with a way to recover or reset their password if they have lost or forgotten it. This is typically presented as a hyperlink below the credential fields. Clicking the link will present users with a field to enter their e-mail address. They will receive an email with a hyperlink to reset their password. This workflow involves the following steps:
...
When the users are logged in, they have the option to change their password by going through the change password workflow, typically triggered within the user settings. This workflow includes the following steps:
...
Email communications are frequently used throughout the user authentication workflow, as they are the primary way a user will get notified without an account access. Users receive emails for invitations, verification, confirmation codes, etc.
...
The purpose of a welcome screen is to assure the user that something has been achieved. These screens can be an opportunity to perform more product branding — you are encouraged to use animations and [graphics](/style/illustrations) on these screens.
