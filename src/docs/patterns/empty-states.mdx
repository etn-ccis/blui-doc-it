...
The empty state pattern should communicate to the users clearly about:

-   What they are looking at: What data can possibly show up if the dataset is not empty.
-   Why they end up here: What they did previously that causes the dataset to be empty, if any.
-   How to get out of here: What actions do they need to take for data to populate.

<ImageGrid
    images={[DontDistractImage, DoEncourageImage]}
    caption={
        'Do not use table headers and paginations to distract your user as they are not in active use. Instead, provide instructions to help your user out of the empty state situation.'
    }
/>

Empty states can also be a great place to express your project identities, especially if your application is designed to be consumer-facing. See [Project Identity](/design/project-identity#illustrations) for details.
