...
<ImageGrid images={[PermanentImg]} style={{marginBottom: 32}}/>
...
<ImageGrid images={[TemporaryImg]} style={{marginBottom: 32}}/>
...
## Navigation Rail / Responsive Navigation

<ImageGrid images={[PersistentImg, PermanentImg]} style={{marginBottom: 32}} caption={'When the user hovers on the Navigation Rail, the side navigation will expand temporarily, revealing the text details of each navigation item; when the user moves their cursor away, the whole side navigation panel collapses.'} />

<DemoCard repository={'icon-navigation'} angular react float={'right'}/>

The Navigation Rail is a minimalist approach to the navigation panel. It features a single icon to represent each of the main areas of the application. This navigation panel is typically used on larger screens only, and is expandable to a full side navigation panel on hover or when clicking the menu icon. 
