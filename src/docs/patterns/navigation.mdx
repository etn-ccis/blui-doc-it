...
For mobiles and tablets, you may use bottom tabs to replace the classical drawer navigations. Bottom tabs are a quick and easy way to navigate to the most frequently used primary navigation routes.

Bottom tabs can also be used with a drawer. The tab is more prominent and can be used to highlight some main routes and features, while the drawer handles less important routes like settings, legal documents, FAQ pages.

<React.Fragment>
    <ImageGrid
        images={[NavigationBottomWithDrawerImg]}
        caption={['A temporary content-cover menu (#1) combined with a bottom tab navigation (#2) on mobile.']}
        regularWidth
    />
    <Divider />
</React.Fragment>
