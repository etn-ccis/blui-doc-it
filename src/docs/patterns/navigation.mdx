...
<ImageGrid images={[NavigationBreakdownImg]} regularWidth />

Most drawers have four main sections:

-   **Header**: contains the name of the company, name of the product or a logo
-   **Subheader**: optional section that can be used to support custom content, such as filtering or search
-   **Body**: contains a list of primary pages that users can navigate too
-   **Footer**: contains company logo, version number and trademark information
...
-   fewer than seven top-level pages
-   primary pages with labels that require complex icons or may be too long to wrap effectivly
...
Tab navigation uses a horizontal bar with buttons for each navigation location. These can appear at the top or bottom of your application. On desktop and tablet, tabs at the top can be used in conjunction with drawer navigation as a way to navigate between sub-pages (e.g., you may have several sub-pages on your main settings page). Bottom tabs can be used in conjuction but is recommended that this pattern replaces the drawer navigation. On mobile it is also possible to combine top and bottom tabs on a single page.

<ImageGrid images={[NavigationTabsImg]} regularWidth />
...
<ImageGrid images={[NavigationRailSpecsImg, NavigationRailLabeledImg, PanelSpecsImg, NavigationTabsSpecsImg]} regularWidth />
