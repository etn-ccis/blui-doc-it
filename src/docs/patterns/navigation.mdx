...
<React.Fragment>
    ...
    <TOC
        anchors={[
            ...
            { title: 'Drawer Navigation', hash: '#drawer-navigation' },
            { title: 'Tab Navigation', hash: '#tab-navigation' },
...
<React.Fragment>
    ...
    <ImageGrid
        ...
        caption={[
            'Be careful when using persistent drawer with drawer item group headers (where it says "GROUP 1" and "GROUP 2" in the image), as they turn into blank spaces when the drawer collapses.',
            `One way to get around this issue is to use different drawer variants at different view ports. For example, you can use a permanent drawer on desktop screens (#1) and switch to a persistent drawer without header text or a navigation rail on tablets (#2).`,
        ]}
    />
</React.Fragment>
