...
import { FontDownloadButton } from '../../../app/components';
...
For detailed usage instructions, refer to the [Brightlayer UI React Native Templates documentation](https://github.com/etn-ccis/blui-react-native/blob/master/packages/cli-templates/README.md).

> **NOTE**: If you encounter peer-dependency errors when installing dependencies, first verify that you are using a supported combination of React Native and Brightlayer UI template versions as documented in the templates README, and try using `yarn install` (which respects peer dependencies). As a last-resort workaround, you may run `npm install --legacy-peer-deps`, but be aware that this disables peer-dependency enforcement and can lead to an incompatible and hard-to-debug dependency graph.
...
The Brightlayer UI React Native templates use the Open Sans font family. Click the button below to download all the required font files as a single zip:
...
```sh
npm install --save @brightlayer-ui/react-native-components
// or
