...
<FAQExpander question={`Why do I need to configure a proxy while working from Mainland China?`}>

You need a proxy in Mainland China because national network restrictions block or slow access to global developer platforms, so routing through Eaton's approved proxy ensures stable and secure connectivity.

More details: [Developer Documentation Access Issues in Mainland China](https://eaton-corp.atlassian.net/wiki/x/NYPKYQ).
