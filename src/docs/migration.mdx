...
<!-- KEYWORDS: developer package migration guide -->
...
-   Change package namespace from `@pxblue` to `@brightlayer-ui`.
-   The below code snippets indicate the changes to be made for using Brightlayer UI CLI command.
...
-   Change all `pxb` reference to `blui` from CLI command.

    ```
    pxb help` => `blui help`
    pxb version` => `blui version`
    pxb new => blui new
    ```
...
-   Replace `@pxblue` reference to `@brightlayer-ui` in import statement in your application.
